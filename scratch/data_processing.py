...
# meta data
def read_meta_merfish_cell_line(
    input_path, behavior_no_space, sex, l_u, r_u, top_k_response_de=10
):
    """
    ...
    de_genes = [
        g.upper().strip() for g in de_genes
...
def read_merfish_cell_line_data(
    input_path, bregma, animal_id, genes_list, genes_list_u=None, cutoff=10050
):
    """
...
def read_starmap_combinatorial(
    input_path, bregma, animal_id, genes_list, genes_list_u=None
):
...
the same condition (say, all naive animals)
    ...
    if data_type == "seqfish":
        ...
        for fov in samples_train:
            idx_train_general = idx_train_general + list(
                meta_all.index[meta_all["Field of View"] == fov].values
            )

        idx_test_general = []
        for fov in samples_test:
            idx_test_general = idx_test_general + list(
                meta_all.index[meta_all["Field of View"] == fov].values
            )
        ...
        if current_cell_type == "general":
            ...
            idx_train = []
            for fov in samples_train:
                idx_train = idx_train + list(
                    meta_all.index[
                        (meta_all["Field of View"] == fov)
                        & (meta_all["cell_types"] == current_cell_type)
                    ]
                )

            idx_test = []
            for fov in samples_test:
                idx_test = idx_test + list(
                    meta_all.index[
                        (meta_all["Field of View"] == fov)
                        & (meta_all["cell_types"] == current_cell_type)
                    ]
        ...
        for animal in samples_train:
            idx_train_general = idx_train_general + list(
                np.where(meta_all[:, meta_all_columns["Animal_ID"]] == animal)[0]
            )
        ...
        if current_cell_type == "general":
            ...
            idx_test, idx_test_in_dataset, meta_per_dataset_test = get_idx_per_dataset(
                samples_test, meta_all, meta_all_columns, bregma
            )
        else:
            (
                idx_train,
                ...
                samples_train, meta_all, meta_all_columns, bregma, current_cell_type
            )
            idx_test, idx_test_in_dataset, meta_per_dataset_test = get_idx_per_dataset(
                samples_test, meta_all, meta_all_columns, bregma, current_cell_type
            )
        ...
        print(
            "Please provide a data type. Only 'seqfish','merfish' or 'starmap' is supported."
        )

        return None, None, None, None, None, None, None, None


def get_idx_per_dataset_merfish_cell_line(
    samples, meta_all, meta_all_columns, single_bregma=None, current_cell_type=None
):
    """
...
def get_idx_per_dataset_starmap_combinatorial(
    samples, meta_all, meta_all_columns, single_bregma=None, current_cell_type=None
):
    """
...
def get_idx_per_dataset_merfish(
    samples, meta_all, meta_all_columns, single_bregma=None, current_cell_type=None
):
    """
    ...
    for animal in samples:
        ...
        for bregma in _bregmas:
            ...
            if current_cell_type is not None:
                _condition = (
                    (meta_all[:, meta_all_columns["Animal_ID"]] == animal)
                    & (meta_all[:, meta_all_columns["Cell_class"]] == current_cell_type)
...
def find_neighbors(pindex, tri):
    ...
    return tri.vertex_neighbor_vertices[1][
        tri.vertex_neighbor_vertices[0][pindex] : tri.vertex_neighbor_vertices[0][
            pindex + 1
        ]
    ]
...
def get_neighbors_datasets(
    data_sets, neighbor_type, k=10, dis_filter=100, include_self=True
):
    """
    ...
    for i in range(len(data_sets)):
        ...
        if neighbor_type == "Del":
            _nn = get_Delaunay_neighbors(
                current_cor, dis_filter=dis_filter, include_self=include_self
            )

        elif neighbor_type == "k-nn":
            _nn_g = kneighbors_graph(
                current_cor, k=k, mode="connectivity", include_self=include_self
            )
            _nn = _nn_g.toarray()
...
def prepare_features(
    ...
    # initialize
    X_trains = {
        "regulators_neighbor": None,
        "regulators_self": None,
        "neighbor_type": None,
        "baseline": None,
        "regulators_neighbor_self": None,
        "regulators_neighbor_by_type": None,
    }
    X_tests = {
        "regulators_neighbor": None,
        "regulators_self": None,
    ...
    for i in range(0, len(feature_types)):
        ...
        X_train = preprocess_concatenate_X_datasets(
            data_sets,
            meta_per_dataset,
            data_type,
            current_idxs,
        ...
        X_test = preprocess_concatenate_X_datasets(
            data_sets,
            meta_per_dataset,
            data_type,
            current_idxs,
    ...
    if top_k_regulator is not None:
        ...
        for key in ["regulators_neighbor", "regulators_neighbor_self"]:
            if X_trains[key] is not None:
                ...
                regulator_list_neighbor = list(
                    np.array(regulator_list_neighbor)[idx_filtered]
                )

        # filter regulators self
        dispersion = X_trains["regulators_self"].var(axis=0) / (
            X_trains["regulators_self"].mean(axis=0) + 1e-6
        )
        idx_filtered = np.argsort(-dispersion, axis=0)[:top_k_regulator]
...
def preprocess_concatenate_X_datasets(
    data_sets,
    meta_per_dataset,
    data_type,
    current_idxs,
...
def preprocess_X_neighbor_by_type_per_cell(
    i,
    data_type,
    current_meta,
    meta_columns,
    ...
    for cur_type in set(sub):
        _per_type[keys.index(cur_type), :] = np.sum(
            nb_genes_fil[np.where(sub == cur_type)[0], :], axis=0
        )
...
def preprocess_X_neighbor_per_cell(
    i,
    data_type,
    current_meta,
    meta_columns,
...
def preprocess_X_self_per_cell(
    i,
    data_type,
    current_meta,
    meta_columns,
...
def preprocess_X_neighbor_type_per_dataset(
    data_type,
    current_meta,
    meta_columns,
    current_cor,
...
def preprocess_X_baseline_per_dataset(
    data_type,
    current_meta,
    meta_columns,
    current_cor,
    ...
    if data_type == "seqfish":
        ...
        dummies = pd.get_dummies(
            current_meta["Cell_class"][current_idx], prefix="", prefix_sep=""
        )
        full_cell_types = list(cell_types_dict.keys())
        ...
        if current_meta is not None:
            ...
            # for the categories not present, fill them all 0
            dummies = pd.get_dummies(
                current_meta[current_idx, meta_columns["Cell_class"]],
                prefix="",
                prefix_sep="",
            )
            cell_type_one_hot = dummies.T.reindex(full_cell_types).T.fillna(0)
        ...
        if current_meta is not None and current_cor is not None:
            ...
            raise TypeError(
                "Both cell type info and coordinates info NOT available! No baselie featues "
                "should be generated"
            )
            return None
    else:
...
def prepare_responses(
    ...
    Y_train, Y_train_true, transformer = preprocess_Y_datasets(
        data_sets,
        data_type,
        current_idxs,
    ...
    Y_test, Y_test_true, _ = preprocess_Y_datasets(
        data_sets,
        data_type,
        current_idxs,
        current_idx_in_general,
...
def preprocess_Y_datasets(
    ...
    if response_type == "original":
        ...
        if response_type == "residual":
            ...
            temp = Y_dif[feature_list].apply(
                lambda x: pd.cut(x, bins, labels=[0, 1, 2]), axis=1
            )
...
def combine_features(features, preprocess, num_coordinates):
    ...
    if "neighbor" not in preprocess:
        ...
        if "baseline" in preprocess:
            ...
            X = np.concatenate(
                (features["regulators_neighbor_self"], features["regulators_self"]),
                axis=1,
            )
            X_clf = X
            X_full = X
        ...
        if "neighbor_cat" in preprocess:
            X = np.concatenate(
                (
                    features["regulators_neighbor_self"],
                    features["regulators_self"],
                    features["regulators_neighbor"],
                ),
                axis=1,
            )
        ...
        if features["neighbor_type"] is not None and features["baseline"] is not None:
            X_clf = np.concatenate(
                [features["baseline"][:, :num_coordinates], features["neighbor_type"]],
                axis=1,
            )
