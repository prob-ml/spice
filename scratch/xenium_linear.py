    ...
    for r in tqdm(r_values, desc="Radius values"):
        ...
        for i, response_gene in enumerate(response_gene_names, start=1):
            ...
            for model, model_name in zip(models, model_names):
                ...
                training_mse[r][response_gene][model_name] = train_mse
                testing_mse[r][response_gene][model_name] = test_mse

    # Save results for this fold
    with open(f"xenium_linear_train_FOLD={fold}.json", "w", encoding="utf-8") as f:
        json.dump(training_mse, f)

    with open(f"xenium_linear_test_FOLD={fold}.json", "w", encoding="utf-8") as f:
